...
# =====================================================================
# 3) Toxic-ish patterns (EXPANDED + stronger)
# =====================================================================

TOXIC_PATTERNS = [
    # insults / profanity (word boundaries, light punctuation variants)
...
def is_toxic(text: str) -> bool:
    """
    Return True if text matches any toxic pattern.
    Note: This ignores prior label/compound and purely checks the text.
    """
    t = (text or "").lower()
...
def analyze_text(text: str) -> AnalyzeOut:
    ...
    # Final outputs (initial)
    label = label_from_compound(comp)                  # 0.62 / 0.44 thresholds
    conf = confidence_from_label_and_compound(label, comp)  # 0..1

    # ---------- TOXIC OVERRIDE (wins over pos/neu/neg) ----------
    toxic_flag = is_toxic(text)
    if toxic_flag:
        # Ensure clearly negative orientation for visibility (cap at -0.60 minimum)
        if comp > -0.60:
            comp = -0.60
        label = "toxic"
        # Confidence remains a function of compound
        conf = confidence_from_label_and_compound("negative", comp)
